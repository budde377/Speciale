...
%The analysis raises multiple warnings. provides an error to bring attention to the possibly bad design choice as seen in figure \ref{fig:directoryScreenshot}. The mixing of array types are detected by the analysis because the append operation creates a list array and then afterwards a write is encountered with the index possibly being a string which then triggers the error.
...
\end{subfigure}
\caption{Running analysis on \texttt{directory\_map} \todo{Crop figures}}
\label{fig:directoryScreenshot}
\end{figure}

\todo{new figures}

\todo{consider newpage}
\newpage
%\begin{figure}[htbp]
%\centering
