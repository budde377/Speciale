\section{Coercion}
\todo{description first, definition afterwards. More description of interesting cases of coercion}
In order to mimic the ability of coercing values the following functions
\begin{align*}
c_{\alpha, \beta} : \alpha \rightarrow \beta
\end{align*}
where $\alpha \in \{ \text{Value}, \text{Array}, \text{Number}, \text{String}, \text{Null}, \text{Boolean}\}$ and {
\def\OldComma{,}
    \catcode`\,=13
    \def,{%
      \ifmmode%
...
$\beta \in \{\text{Number}, \text{Value}, \text{String}, \text{Index}, \text{Boolean}\}$ } are defined as 
\begin{align*}
c_{\alpha,\text{Value}}(v) &=   \begin{cases} 
  ...
  \end{cases}\\
  c_{\text{Value}, \alpha}((v_1, v_2, v_3, v_4, v_5)) &= 
  c_{\text{Array}, \alpha}(v_1) \sqcup c_{\text{String}, \alpha}(v_2) \\ 
  &\sqcup c_{\text{Number}, \alpha}(v_3) \sqcup c_{\text{Boolean},\alpha }(v_4) \sqcup c_{\text{Null}, \alpha}(v_5)\\
\end{align*}
...
A few things are worth noticing about coercion in PHP. 
\begin{itemize}
...
Comparison operations are defined directly on values since different types can be compared with each other courtesy of type translation. PHP has an ordered definition of how values are compared depending on their type which can be seen in table \ref{tab:comparisons}.
