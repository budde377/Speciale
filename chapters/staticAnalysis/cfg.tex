...
\begin{graph}
\centering
\begin{adjustbox}{max size={1\textwidth}{.25\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
	\node [entry] (en) {};
 	\node [node, right of=en, label={270:$\mathit{const_r}(\texttt{1},t_1)$}] (c1) {};
 	\node [node, right of=c1, label={90:$\mathit{const_r}(\texttt{2},t_2)$}] (c2) {};
 	\node [node, right of=c2, label={270:$\mathit{const_r}(\texttt{3},t_3)$}] (c3) {};
 	\node [node, right of=c3, label={90:$\mathit{bop}_+(t_2,t_3, t_4)$}] (bop1) {};
 	\node [node, right of=bop1, label={270:$\mathit{bop}_+(t_1,t_4, t)$}] (bop2) {};
...
\captionsetup{justification=centering}
\hspace*{\fill}
\subcaptionbox{Function call \label{graph:flowcall}}{\begin{adjustbox}{max size={.3\textwidth}{.40\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
    \node [entry] (en) {};
...
\subcaptionbox{Variable write\label{fig:vwrite}}{\begin{adjustbox}{max size={.24\textwidth}{.25\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [node, below of=e2, label={0:$\mathit{var}_w(\texttt{\$$$v},c_{val},t_{tar})$}] (ass) {};
...
\item[$call_{\mathit{fn}}(L_1,\dots,t_n)$] a function call with parameters $t_1$ through $t_n$, this node have corresponding \emph{exit} and \emph{result} nodes that handle passing information to functions and passing the result back 
...
\item[$\mathit{var}_w(v,t_{val},t_{tar})$] write the value of variable $t_{val}$ to $v$ and store it at stack variable $t_{tar}$.
\item[$\mathit{var}_w(v,h_{val},t_{tar})$] ? 
\item[$\mathit{var}_r(v,t_{tar})$] read the variable $v$ into the temporary variable $t_{tar}$
\item[$\mathit{var}_r(v,h_{tar})$] ? 
