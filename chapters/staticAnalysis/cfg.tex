...
\item{$\mathit{varRead}(\texttt{\$v}, c_{tar})$, $\mathit{var}_w(\texttt{\$v}, c_{val}, t_{tar})$:} These nodes indicates reading from and writing to a variable, \texttt{\$v}, respectively. Depending on the context the target of the read and value of the write can either be a temporary or temporary heap storage. 
...
\begin{adjustbox}{max size={.33\textwidth}{.33\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
	...
	\node [node, below of =t3, label={-45:$array_w(h_{var}, t_{key}, h_{val}, t)$}] (ass) {};
	\node [exit, below of=ass] (ex) {};
    % Draw edges
    \path [line] (en) -> (t1);
    \path [line] (t1) -> (t2);
...
\begin{adjustbox}{max size={.33\textwidth}{.33\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
	...
	\node [entry] (en) {};
	\node [subtree, below of =en](t1) {	 $\subt{\texttt{\$$$a}}(h_{var'})$};
	\node [node, below of = t1, label={-45:$array_a(h_{var'}, h_{var})$}] (aa) {};
	\node [node, below of = aa, label={-45:$const_r(\texttt{'foo'}, t_{key})$}] (cr) {};		
	\node [subtree, below of =cr](t3) { $\subt{\texttt{func()}}(h_{val'})$};
	\node [node, below of = t3, label={-45:$array_a(h_{val'}, h_{val})$}] (aa2) {};
	\node [node, below of =aa2, label={-45:$array_w(h_{var}, t_{key}, h_{val}, t)$}] (ass) {};
	\node [exit, below of=ass] (ex) {};
    % Draw edges
    \path [line] (en) -> (t1);
    \path [line] (t1) -> (aa);
...
\begin{adjustbox}{max size={.33\textwidth}{.33\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
	\node [entry] (en) {};
	\node [node, below of =en, label = {-45: $var_r(\texttt{\$$$a}, h_{var'})$}](t1) {};
	\node [node, below of = t1, label={-45:$array_a(h_{var'}, h_{var})$}] (aa) {};
	\node [node, below of = aa, label={-45:$const_r(\texttt{'foo'}, t_{key})$}] (cr) {};		
