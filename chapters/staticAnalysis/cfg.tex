...
\begin{adjustbox}{max size={1\textwidth}{.25\textheight}, margin = 10pt}\begin{tikzpicture}[node distance = 2cm, auto]
	...
	\node [entry] (en) {};
 	\node [node, right of=en, label={270:$\mathit{const_r}(\texttt{1},t_1)$}] (c1) {};
 	\node [node, right of=c1, label={90:$\mathit{const_r}(\texttt{2},t_3)$}] (c2) {};
 	\node [node, right of=c2, label={270:$\mathit{const_r}(\texttt{3},t_4)$}] (c3) {};
