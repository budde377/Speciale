...
\subcaptionbox{\label{fig:flowret}}{\begin{adjustbox}{max size={.3\textwidth}{.25\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
    % Place nodes
    \node [entry] (en) {};
    \node [subtree, below of=en] (exp) {$E(t)$};
    \node [node, below of=exp, label={45:$exit(\cdots, t, \cdots)$}] (exitn) {};
    \node [emptynode, below of=exitn ] (end) {};
    \node [exit, below of=end] (ex) {};
    % Draw edges
    \path[line] (en) -> (exp);
...
\end{tikzpicture}\end{adjustbox}}\hfill%
\subcaptionbox{\label{fig:flowrret}}{\begin{adjustbox}{max size={.3\textwidth}{.25\textheight}}\begin{tikzpicture}[node distance = 2cm, auto]
  % Place nodes
    \node [entry] (en) {};
    \node [subtree, below of=en] (exp) {$Re(h)$};
    \node [node, below of=exp, label={45:$exit(\cdots, h, \cdots)$}] (exitn) {};
    \node [emptynode, below of=exitn ] (end) {};
    \node [exit, below of=end] (ex) {};
    % Draw edges
    \path[line] (en) -> (exp);
...
For $e = \texttt{\$v}$ for some variable \texttt{\$v}, the graph is a single $var_r(\texttt{\$v}, t_{tar})$ node. For an array read expression, $e = E_{ar}\texttt[E_{key}\texttt{]}$, the sub array expression, $E_{ar}$, should be evaluated before the key expression, $E_{key}$, and the graph then becomes like figure \ref{fig:flowaread}.
\begin{figure}
\ContinuedFloat
\hspace*{\fill}
